...
[LocalLLMPlugin.kt](plugins-src/local-llm-plugin/src/android/LocalLLMPlugin.kt) right now.
...
- [llama_bridge.cpp](plugins-src/local-llm-plugin/src/android/cpp/llama_bridge.cpp)
  appends a literal `/no_think` suffix to every prompt (Qwen3's documented way to
  request non-thinking mode).
...
The filename must exactly match what
[LocalLLMPlugin.kt](plugins-src/local-llm-plugin/src/android/LocalLLMPlugin.kt)'s
...
3. Update the filename in `ensureModelLoaded()` in
   [LocalLLMPlugin.kt](plugins-src/local-llm-plugin/src/android/LocalLLMPlugin.kt).
...
[LocalLLMPlugin.kt](plugins-src/local-llm-plugin/src/android/LocalLLMPlugin.kt)'s
`ensureVisionModelLoaded()`, used by the "Take photo"/"Choose photo" buttons.
Stepped up from SmolVLM-500M after it badly misidentified a test photo (a coffee mug
called a "tree or person") — same size-vs-accuracy tradeoff as the text model.
...
struct in [llama_bridge.cpp](plugins-src/local-llm-plugin/src/android/cpp/llama_bridge.cpp)'s
`loadVisionModel()`:
```cpp
